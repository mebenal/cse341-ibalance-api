...
module.exports.addTask = function (category, title, date, completed, notes, user, repeat) {
  let repeatId;
  if (repeat) {
...
module.exports.editTask = function (taskId, category, title, date, completed, notes) {
  Task.findById(taskId)
  .then(task => {
...
module.exports.getTasks = function (startDate, endDate, user) {
  return Task.find({ date: { $gte: startDate, $lte: endDate }, userId: user._id })
}
